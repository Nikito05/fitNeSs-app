...
- **Selector de tema sin `next-themes`** (`docs/superpowers/specs/2026-08-19-selector-de-tema-design.md`): se implementó el selector Claro/Oscuro/Sistema a mano (`src/lib/theme.ts` + `ThemeProvider` + un script bloqueante inline en el `<head>` de `layout.tsx`) en vez de agregar la dependencia `next-themes`, siguiendo el mismo criterio ya usado para `Tamaño de letra` (`src/lib/font-size.ts`) — coherencia con el patrón existente en el proyecto antes que sumar una librería para algo de alcance chico. El script bloqueante evita el flash de tema incorrecto al cargar (duplicando a propósito la lógica de `resolveTheme`, ver el comentario en `layout.tsx`).
- **Primer color semántico verde del proyecto, para "meta cumplida" en las barras de progreso de Macros** (`docs/superpowers/specs/2026-08-19-macros-barras-progreso-design.md`): las barras de `/macros` introducen `green-600`/`dark:green-500` (barra llena) como primer uso de verde semántico en la app — sigue el mismo patrón que `text-destructive`/`text-amber-600` (colores estándar de Tailwind con rol semántico fijo, no tonos custom). El texto del badge de exceso usa un tono más oscuro (`green-700`/`dark:green-400`) que la barra, porque el `green-600` plano no llega a contraste AA sobre el fondo claro del badge a ese tamaño de texto chico — la barra en sí no tiene ese problema porque no lleva texto encima. De paso, `calculateRemaining` (`src/lib/comidas/food-calculation.ts`) quedó sin consumidor de pantalla tras esta rama (`MacroProgress` calcula su propio remanente internamente) — se deja como utilidad de dominio con sus tests existentes, sin borrar, por si se reutiliza en una futura vista (ej. un resumen semanal).
